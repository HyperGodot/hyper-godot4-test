...
func _ready():
	...
	var callable = Callable(self, "_on_got_latest")
	latestInfoRequest.connect("request_completed", callable)
	
	add_child(downloadFileRequest)
	callable = Callable(self, "_on_finished_download")
	downloadFileRequest.connect("request_completed", callable)
...
func getExecutibleName():
	...
	if OSname == 'Linux': return "hyper-gateway-linux"
	if OSname == 'OSX': return "hyper-gateway-macos"
	if OSname == 'Windows': return "hyper-gateway-windows.exe"
	push_error("Can not start gateway, invalid operating system " + OSname)
